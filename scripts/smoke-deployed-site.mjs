...
const presentationIsCurrentRelease =
  servedWebsiteSha === expectedWebsiteSha
  && servedPresentationSha === release.presentations.sha;

if (presentationIsCurrentRelease) {
  if (servedGptApiBase !== productionGptApiBase) {
    throw new Error(
      `current presentation GPT API base is ${servedGptApiBase || 'missing'}, expected ${productionGptApiBase}`,
    );
  }

  console.log(`Presentation release metadata matches ${releaseFlag}`);
  console.log(`Verified direct GPT API ${productionGptApiBase}`);
} else if (shaPattern.test(servedWebsiteSha) && shaPattern.test(servedPresentationSha)) {
  console.log(
  ...
  console.log(
    `::warning::Presentation CDN is serving a cached deck from before release stamping was introduced; `
      ...
      + `The uploaded Pages artifact was validated before deployment.`,
  );
}

console.log(`Smoke tested ${baseUrl.origin} at ${releaseFlag}`);
console.log(`Verified presentation route ${presentationUrl.pathname}`);
